...
assert.doesNotMatch(feed, /data-emoji-picker-panel[\s\S]*Comments Section/, 'Feed emoji picker should not be inline above comments');
assert.doesNotMatch(singlePost, /data-emoji-picker-panel[\s\S]*Comments Section/, 'SinglePost emoji picker should not be inline above comments');
assert.match(feed, /fixed inset-0[\s\S]*role="dialog"[\s\S]*data-emoji-picker-panel/, 'Feed emoji picker should render as a fixed modal dialog');
assert.match(singlePost, /fixed inset-0[\s\S]*role="dialog"[\s\S]*data-emoji-picker-panel/, 'SinglePost emoji picker should render as a fixed modal dialog');
...
assert.match(feed, /selectedPost\.comment[\s\S]*selectedPost\.hashtags[\s\S]*selectedPost\.createdAt[\s\S]*Comments Section[\s\S]*border-t[\s\S]*data-emoji-picker[\s\S]*selectedPost\.comments/, 'Feed reactions should sit below the post/comment separator and above comments');
assert.match(singlePost, /post\.comment[\s\S]*post\.hashtags[\s\S]*post\.createdAt[\s\S]*Comments Section[\s\S]*border-t[\s\S]*data-emoji-picker[\s\S]*post\.comments/, 'SinglePost reactions should sit below the post/comment separator and above comments');
