...
assert.match(feed, /showCustomEmojiUploadForm/, 'Feed should keep custom emoji upload behind a separate menu button');
assert.doesNotMatch(feed, /オブジェクトストレージに保存/, 'Feed UI must not expose storage implementation details to users');
assert.match(feed, /onError=\{[^}]*handleCustomEmojiImageError/, 'Feed should hide broken custom emoji images instead of showing the browser broken-image icon');
...
assert.match(singlePost, /showCustomEmojiUploadForm/, 'SinglePost should keep custom emoji upload behind a separate menu button');
assert.doesNotMatch(singlePost, /オブジェクトストレージに保存/, 'SinglePost UI must not expose storage implementation details to users');
assert.match(singlePost, /onError=\{[^}]*handleCustomEmojiImageError/, 'SinglePost should hide broken custom emoji images instead of showing the browser broken-image icon');
